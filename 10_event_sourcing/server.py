"""Phase 10: Event Sourcing for Loans (alongside everything else from previous phases).

The new event-sourced Loan aggregate has its own endpoints under /es/loans/...
State is rebuilt by replaying the event stream (with periodic snapshots).
Old event versions are upcast on read.
"""
...
from adapters.event_store import create_event_store_schema, map_event_store
...
from service_layer import loan_service
...
PORT = 8010
DB_FILE = os.environ.get("LIBRARY_DB", "library_phase10.db")
...
create_event_store_schema(engine)
...
app = FastAPI(title="Library -- Phase 10: Event Sourcing", lifespan=lifespan)
...
class EsBorrowIn(BaseModel):
    loan_id: str
    member_id: str
    isbn: str


class EsLoanIn(BaseModel):
    loan_id: str


@app.post("/es/loans/borrow")
def es_borrow(payload: EsBorrowIn) -> dict:
    try:
        return loan_service.borrow_loan(
            SessionFactory,
...
@app.post("/es/loans/renew")
def es_renew(payload: EsLoanIn) -> dict:
    try:
        return loan_service.renew_loan(SessionFactory, loan_id=payload.loan_id)
    except Exception as exc:
        raise HTTPException(409, str(exc)) from exc


@app.post("/es/loans/return")
def es_return(payload: EsLoanIn) -> dict:
    try:
        return loan_service.return_loan(SessionFactory, loan_id=payload.loan_id)
    except Exception as exc:
        raise HTTPException(409, str(exc)) from exc


@app.get("/es/loans/{loan_id}")
def es_get(loan_id: str) -> dict:
    return loan_service.get_loan(SessionFactory, loan_id=loan_id)


@app.get("/es/loans/{loan_id}/history")
def es_history(loan_id: str) -> dict:
    return {"events": loan_service.get_history(SessionFactory, loan_id=loan_id)}
