...
from domain.model import Book, BranchStock, ISBN, Loan, Member, Reservation, Transfer
...
branch_stock_table = Table(
    ...
    UniqueConstraint("isbn", "branch", name="uix_isbn_branch"),
)
...
transfers_table = Table(
    ...
    metadata,
    Column("id", Integer, primary_key=True, autoincrement=True),
    Column("transfer_id", String(64), nullable=False, unique=True),
    Column("isbn", ISBNType, nullable=False),
    Column("from_branch", String(50), nullable=False),
    Column("to_branch", String(50), nullable=False),
    Column("state", String(20), nullable=False),
    Column("failure_reason", String(200), nullable=True),
    Column("simulate_receive_failure", Boolean, nullable=False, default=False),
)
...
def map_domain() -> None:
    ...
    mapper_registry.map_imperatively(BranchStock, branch_stock_table)
    mapper_registry.map_imperatively(Transfer, transfers_table)
    mapper_registry.map_imperatively(
        Member,
        members_table,
        properties={
