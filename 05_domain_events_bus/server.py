...
Aggregates record events while changing state. After commit, the UoW drains
those events and the message bus dispatches them to handlers. Handlers run in
the same process; failure of one handler does not roll back the transaction.
"""
...
from service_layer import handlers, messagebus, services
from service_layer.unit_of_work import make_uow_factory


PORT = 8005
DB_FILE = os.environ.get("LIBRARY_DB", "library_phase5.db")
...
handlers.register_all(messagebus.register)


app = FastAPI(title="Library — Phase 5: Domain Events + Message Bus")
