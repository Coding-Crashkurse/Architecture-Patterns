...
Write side stays exactly as Phase 7 (commands -> bus -> aggregates -> UoW -> outbox).
Read side gets new endpoints (/views/...) that go directly against the DB with raw
SELECTs, returning shapes optimised for the consumer instead of the domain model.
"""
...
PORT = 8008
DB_FILE = os.environ.get("LIBRARY_DB", "library_phase8.db")
...
app = FastAPI(title="Library -- Phase 8: CQRS read paths", lifespan=lifespan)
